...
def main(strace_output, sst_dump, thread_count):
    files = []
    regex = re.compile("^\d+ (\d{2}:\d{2}:\d{2}).*\"(.*.sst)\".*$")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--strace-output", help="path to strace output", required=True)
    ...
    main(args.strace_output, args.sst_dump, int(args.threads))
